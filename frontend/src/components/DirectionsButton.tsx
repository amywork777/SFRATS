...
function DirectionsButton({ lat, lng, address, className = '', variant = 'default' }: DirectionsButtonProps) {
  ...
  const styles = variant === 'prominent'
    ? 'bg-bridge-500 text-paper-light border border-ink shadow-stamp px-4 py-2 hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0_0_rgba(24,22,19,1)] transition-all font-mono text-[11px] uppercase tracking-[0.14em] font-semibold'
  ...
  return (
    <button onClick={handleClick} className={`inline-flex items-center gap-2 ${styles} ${className}`}>
      <MapIcon size={14} strokeWidth={2.2} /> Open in Google Maps
    </button>
  )
}
